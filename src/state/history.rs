...
mod tests {
    ...
    async fn mutation_delete_timestamp() {
        ...
        let k = common::BlockNumber(1).db_key();
    ...
    async fn mutation_commit() {
        ...
        for i in 0..num_of_accounts {
            ...
            let res_account_storage = txdb::walk(
                ...
                8 * (common::ADDRESS_LENGTH as u64 + 8),
            )
            .fold(HashMap::new(), |mut accum, res| {
                let (k, v) = res.unwrap();
                accum.insert(
                    common::Hash::from_slice(&k[common::ADDRESS_LENGTH + 8..]),
        ...
        for (i, &address) in addrs.iter().enumerate() {
            for j in 0..num_of_state_keys {
                let key = common::Hash::from_slice(
                    &hex::decode(format!("{:0>64}", i * 100 + j)).unwrap(),
                );
                let value = common::value_to_bytes(common::Value::from(10 + j as u64))
                    .to_vec()
                    .into();
    ...
    >(
        ...
        for i in 0..num_of_accounts {
            ...
            acc_history[i].code_hash = Some(common::Hash::from_slice(
                &hex::decode(format!("{:0>64}", 10 + i)).unwrap(),
            ));
            // acc_history[i].root = Some(common::Hash::from_slice(
            //     &hex::decode(format!("{:0>64}", 10 + i)).unwrap(),
            // ));
            ...
            for j in 0..num_of_state_keys {
                ...
                let new_value = common::Value::from(j as usize);
